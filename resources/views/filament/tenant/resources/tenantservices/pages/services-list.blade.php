    ...
    <div class="grid grid-cols-4 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-6">
        @foreach ($this->getServices() as $service)
            @php
               // dd($service);
            @endphp
            <a href="{{ Request::url() }}/{{$service->pivot->id}}/edit"  class="relative group bg-white cursor-pointer rounded-xl overflow-hidden shadow transition transform hover:-translate-y-1 hover:shadow-2xl flex items-center justify-center p-4 shadow grid grid-rows-2 gap-4">
